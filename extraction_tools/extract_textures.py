...
def extract_texture(data: bytearray, file_name: str, pos: int = 0):
    ...
    if image_buffer_offset == image_palette_offset and image_buffer_offset < size:
        ...
        converted_data = decoder.decode_compressed_image(
            image_data, width, height, ImageFormats.BC1_DXT1
        )

    elif image_buffer_offset and image_buffer_offset < size:
        image_data: bytes = data[cursor + image_buffer_offset : cursor + size]
        palette_data: bytes = data[
            cursor + image_palette_offset : cursor + image_buffer_offset
        ]

        if len(palette_data) == 1024:
            image_data = swizzle_gamecube.unswizzle_gamecube(
                image_data, width, height, 8
            )
            converted_data = decoder.decode_indexed_image(
                image_data,
                palette_data,
                width,
                height,
            ...
            print(f"unk palette size: {len(palette_data)}")

    if converted_data is None:
        return

    pil_image = wrapper.get_pillow_image_from_rgba8888_data(
        converted_data, width, height
    )
    ...
    file_name_with_ext = os.path.basename(file_name)

    out_path = os.path.join(parent_dir, "0_text_textures", file_name_with_ext) + ".png"

    print(out_path)

    pil_image.save(out_path)  # type: ignore
    ...
    for file in os.listdir(folder_path):
        ...
        try:
            file_reader = FileHandler(
                file_path=file_path, open_mode="rb", endianess_str=ENDIANESS_STR
            )
            if not file_reader.get_file_size():
